...
const TABLE_NAME = process.env.SECURE_DOC_TABLE;
...
function looksLikeSvg(buf) {
    ...
    if (/^<svg[\s>]/i.test(trimmed))
        return true;
    if (/^<\?xml[\s\S]{0,2048}<svg[\s>]/i.test(trimmed))
        return true;
    return /<svg[\s>]/i.test(trimmed.slice(0, 4096));
}
function mimeForImageKind(kind) {
    switch (kind) {
        case "png":
            return "image/png";
        case "jpeg":
...
function imageKindFromDeclaredOrFilename(declaredCt, filename) {
    const ct = declaredCt.toLowerCase();
    const ext = extFromFilename(filename);
    if (ct.includes("png") || ext === "png")
        return "png";
    if (ct.includes("jpeg") || ct.includes("jpg") || ext === "jpg" || ext === "jpeg")
        return "jpeg";
    if (ct.includes("webp") || ext === "webp")
        return "webp";
    if (ct.includes("gif") || ext === "gif")
        return "gif";
    if (ct.includes("tiff") || ext === "tif" || ext === "tiff")
        return "tiff";
    if (ct.includes("avif") || ext === "avif")
        return "avif";
    if (ct.includes("heic") || ext === "heic")
        return "heic";
    if (ct.includes("heif") || ext === "heif")
        return "heif";
    if (ct.includes("bmp") || ext === "bmp")
        return "bmp";
    if (ct.includes("svg") || ext === "svg")
        return "svg";
    if (ct.includes("x-icon") || ct.includes("vnd.microsoft.icon") || ext === "ico")
        return "ico";
    return "unknown";
}
function looksLikeDocxZip(buf) {
    if (!startsWithBytes(buf, [0x50, 0x4b, 0x03, 0x04]))
        return false;
    const probe = buf.subarray(0, Math.min(buf.length, 512 * 1024)).toString("utf8");
    return (probe.includes("word/document.xml") ||
        probe.includes("word/") ||
        probe.includes("[Content_Types].xml"));
}
function looksLikePagesZip(buf, filename, declaredCt) {
    if (!startsWithBytes(buf, [0x50, 0x4b, 0x03, 0x04]))
        return false;
    ...
    return (probe.includes("index/document.iwa") ||
        probe.includes("metadata/buildversionhistory.plist") ||
        probe.includes("quicklook/preview.pdf"));
}
function detectSourceInfo(rawBuf, srcFilename, srcContentType) {
    ...
    let imageKind = "unknown";
    if (startsWithBytes(rawBuf, [0x89, 0x50, 0x4e, 0x47]))
        imageKind = "png";
    else if (startsWithBytes(rawBuf, [0xff, 0xd8, 0xff]))
        imageKind = "jpeg";
    else if (rawBuf.length >= 12 &&
    ...
    else if (startsWithBytes(rawBuf, [0x49, 0x49, 0x2a, 0x00]) ||
        ...
        if (bmffKind)
            imageKind = bmffKind;
        else if (looksLikeSvg(rawBuf))
            imageKind = "svg";
        else
            imageKind = imageKindFromDeclaredOrFilename(declaredCt, filename);
    }
...
function sanitizeResourceReference(value) {
    const trimmed = value.trim();
    if (!trimmed)
        return "";
    if (trimmed.startsWith("#"))
        return trimmed;
    if (/^data:image\/(?:png|jpeg|jpg|gif|webp|avif|bmp|tiff|x-icon|vnd\.microsoft\.icon);base64,[a-z0-9+/=\s]+$/i.test(trimmed)) {
        return trimmed.replace(/\s+/g, "");
    }
    if (/^(?:https?:|file:|ftp:|javascript:|vbscript:|\/\/|data:)/i.test(trimmed))
        return "";
...
function sanitizeCssUrls(cssText) {
    ...
    return withoutImports.replace(/url\(([^)]*)\)/gi, (match, rawValue) => {
...
function sanitizeSvgResourceAttributes(svg) {
    const sanitizeAttrValue = (attrName, value, quote) => {
        const safe = sanitizeResourceReference(value);
        if (!safe)
            return "";
        return ` ${attrName}=${quote}${safe}${quote}`;
    };
    const withQuotedAttrs = svg.replace(/\s((?:xlink:)?href|src)\s*=\s*(["'])([\s\S]*?)\2/gi, (_match, attrName, quote, value) => sanitizeAttrValue(attrName, value, quote));
    return withQuotedAttrs.replace(/\s((?:xlink:)?href|src)\s*=\s*([^\s>]+)/gi, (_match, attrName, value) => sanitizeAttrValue(attrName, value, '"'));
}
function parseViewBoxDimensions(svgTag) {
...
function sanitizeSvg(buf) {
    ...
    sanitized = sanitized.replace(/<style\b([^>]*)>([\s\S]*?)<\/style>/gi, (_m, attrs, css) => {
        const safeCss = sanitizeCssUrls(css);
        return `<style${attrs}>${safeCss}</style>`;
    });
    sanitized = sanitized.replace(/\sstyle\s*=\s*(["'])([\s\S]*?)\1/gi, (_m, quote, css) => {
...
async function decodeWithPythonSpecial(args) {
    ...
    try {
        ...
        runPythonScript("/var/task/image_special_convert.py", [inPath, outPath, args.target], 60_000, args.label);
...
async function heifToBuffer(buf) {
...
async function avifToBuffer(buf) {
    return decodeWithPythonSpecial({
        inputBuffer: buf,
        inputExt: "avif",
        target: "avif_decode",
...
async function bmpToBuffer(buf) {
    return decodeWithPythonSpecial({
        inputBuffer: buf,
        inputExt: "bmp",
        target: "bmp_decode",
...
function runPythonScript(scriptPath, args, timeoutMs, label) {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
...
async function documentToImagesZip(args) {
    ...
    const baseName = (0, formats_1.stripExt)(args.sourceFilename) || "document";
    ...
    try {
        ...
        const run = runPythonScript("/var/task/document_to_images_zip.py", [
            inPath,
            outZipPath,
            baseName,
            fmtLower,
            ...
            String(resize),
            String(maxWidth),
        ], 120_000, "Document image rendering");
        ...
        const pageCount = typeof pagesRaw === "number" && Number.isFinite(pagesRaw) && pagesRaw > 0
            ? Math.floor(pagesRaw)
            : 0;
        if (pageCount <= 0) {
        ...
        const outputCount = typeof outputCountRaw === "number" && Number.isFinite(outputCountRaw) && outputCountRaw > 0
            ? Math.floor(outputCountRaw)
            : pageCount;
        const packagingRaw = meta?.packaging;
        const packaging = packagingRaw === "single" ? "single" : "zip";
        const fidelityRaw = meta?.fidelity;
        let fidelity;
        if (typeof fidelityRaw === "object" && fidelityRaw !== null) {
            const obj = fidelityRaw;
 ...
 */
async function docxToCanonicalPdf(buf) {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const fs = require("fs");
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const os = require("os");
    ...
    try {
        ...
        const result = fs.readFileSync(outPath);
        if (!result || result.length === 0)
            throw new Error("docx_to_pdf produced an empty output file");
        const meta = parseLastJsonObject(run.stdout) ?? parseLastJsonObject(run.stderr) ?? {};
        const converter = typeof meta.converter === "string" ? meta.converter : "libreoffice";
        const libreofficeVersion = typeof meta.libreoffice_version === "string" ? meta.libreoffice_version : "unknown";
        const pageCount = typeof meta.page_count === "number" && Number.isFinite(meta.page_count)
            ? Math.max(0, Math.floor(meta.page_count))
            : 0;
        const nonBlankPages = typeof meta.non_blank_pages === "number" && Number.isFinite(meta.non_blank_pages)
            ? Math.max(0, Math.floor(meta.non_blank_pages))
            : 0;
        const narrowTextPagesRaw = meta.narrow_text_pages;
        const narrowTextPages = Array.isArray(narrowTextPagesRaw)
            ? narrowTextPagesRaw
                .filter((v) => typeof v === "number" && Number.isFinite(v) && v > 0)
                .map((v) => Math.floor(v))
            : [];
        const conversionStrategy = typeof meta.conversion_strategy === "string" ? meta.conversion_strategy : "baseline";
        ...
        if (typeof sanitizeRaw === "object" && sanitizeRaw !== null) {
            const obj = sanitizeRaw;
            sanitization = {
                applied: obj.applied === true,
                normalizedSections: typeof obj.normalized_sections === "number" && Number.isFinite(obj.normalized_sections)
                    ? Math.max(0, Math.floor(obj.normalized_sections))
                    : 0,
                normalizedParagraphSections: typeof obj.normalized_paragraph_sections === "number" && Number.isFinite(obj.normalized_paragraph_sections)
                    ? Math.max(0, Math.floor(obj.normalized_paragraph_sections))
                    : 0,
                normalizedBodySections: typeof obj.normalized_body_sections === "number" && Number.isFinite(obj.normalized_body_sections)
                    ? Math.max(0, Math.floor(obj.normalized_body_sections))
                    : 0,
                inconsistentColumns: obj.inconsistent_columns === true,
                minColumnWidthSeenTwips: typeof obj.min_column_width_seen_twips === "number" && Number.isFinite(obj.min_column_width_seen_twips)
...
async function pagesToCanonicalPdf(buf) {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const fs = require("fs");
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const os = require("os");
    ...
    try {
        fs.writeFileSync(inPath, buf);
        const run = runPythonScript("/var/task/pages_to_pdf.py", [inPath, outPath], 120_000, "PAGES→PDF conversion");
        const result = fs.readFileSync(outPath);
        if (!result || result.length === 0)
            throw new Error("pages_to_pdf produced an empty output file");
        const meta = parseLastJsonObject(run.stdout) ?? parseLastJsonObject(run.stderr) ?? {};
        ...
        const sourceEntries = Array.isArray(sourceEntriesRaw)
            ? sourceEntriesRaw
...
function extractLargestIcoFrame(buf) {
    ...
    if (bestOffset < 0)
        throw new Error("ICO file contains no valid image frames");
    ...
    if (frame.length < 40)
        throw new Error("ICO BMP frame too small to be a valid DIB header");
...
function sharpOptionsForSource(imageKind) {
    if (imageKind === "svg") {
        return { failOn: "none", density: SVG_DEFAULT_DENSITY };
    }
    return { failOn: "none" };
}
async function preprocessInput(args) {
...
async function applyResize(pipeline, inputBuf, sourceInfo, resizePct, maxWidth) {
    if (typeof resizePct === "number" && resizePct < 100) {
        const meta = await (0, sharp_1.default)(inputBuf, sharpOptionsForSource(sourceInfo.imageKind)).metadata();
...
async function renderNormalizedPng(args) {
    let pipeline = (0, sharp_1.default)(args.inputBuf, sharpOptionsForSource(args.sourceInfo.imageKind));
...
async function convertWithPythonSpecialEncoder(inputPng, target, quality) {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const fs = require("fs");
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const os = require("os");
    ...
    try {
        ...
        runPythonScript("/var/task/image_special_convert.py", scriptArgs, 
        // AVIF encoding via libaom is CPU-intensive; allow up to 2 minutes.
        target === "avif" ? 120_000 : 75_000, `Image→${target.toUpperCase()} conversion`);
        const result = fs.readFileSync(outPath);
        if (!result || result.length === 0) {
...
function pngToEmbeddedSvg(pngBuf, width, height) {
    ...
    if (args.fmt === "ICO" ||
        ...
        if (args.fmt === "HEIC" || args.fmt === "HEIF") {
            const heifQ = typeof args.quality === "number" && Number.isFinite(args.quality)
                ? Math.max(1, Math.min(100, Math.floor(args.quality)))
                : args.config.webpQuality;
            return convertWithPythonSpecialEncoder(normalizedPng, args.fmt === "HEIC" ? "heic" : "heif", heifQ);
        }
        const meta = await (0, sharp_1.default)(normalizedPng, { failOn: "none" }).metadata();
        return pngToEmbeddedSvg(normalizedPng, meta.width ?? 1, meta.height ?? 1);
    }
    let pipeline = (0, sharp_1.default)(inputBuf, sharpOptionsForSource(args.sourceInfo.imageKind));
    pipeline = await applyResize(pipeline, inputBuf, args.sourceInfo, args.resizePct, args.config.maxWidth);
    switch (args.fmt) {
        case "PNG":
...
async function handler(event) {
    ...
    try {
        ...
        const srcRes = await ddb.send(new lib_dynamodb_1.GetCommand({ TableName: TABLE_NAME, Key: { PK, SK: srcSK } }));
        ...
        const getRes = await s3.send(new client_s3_1.GetObjectCommand({ Bucket: srcBucket, Key: srcKey }));
        ...
        if (sourceInfo.kind === "image" && sourceInfo.imageKind === "unknown") {
            throw new Error(`Unsupported or unrecognized source media type (declared content-type: ${srcContentType || "unknown"})`);
        }
        if (!allowedOutputs.includes(outputFormat)) {
            throw new Error(`Unsupported conversion for detected source type ${sourceInfo.kind}/${sourceInfo.imageKind}: ${outputFormat}`);
        }
        ...
        let outputBuf;
        let outputFilename = (0, formats_1.replaceExt)(srcFilename, (0, formats_1.extForFormat)(outputFormat));
        let contentType = (0, formats_1.contentTypeFor)(outputFormat);
        let packaging = "single";
        let pageCount;
        let outputCount;
        let canonicalPdf;
        let pagesCanonicalPdf;
        let pagesCanonicalMeta;
        if (srcIsDocx) {
        ...
        if (outputFormat === "PDF") {
            if (srcIsDocx) {
                ...
                throw new Error(`PDF output for PDF source is not supported (received content-type: ${sourceInfo.mime || srcContentType})`);
            }
            else {
        ...
        else if ((srcIsPdf || srcIsDocx || srcIsPages) && (0, formats_1.isImageOutputFormat)(outputFormat)) {
            const qualityForPages = outputFormat === "JPG"
                    ...
                    : outputFormat === "AVIF"
                        ...
                        : outputFormat === "TIFF"
                            ? (typeof quality === "number" && Number.isFinite(quality)
                                ...
                                : config.tiffQuality)
                            : 90;
            const sourcePdfBuffer = srcIsDocx ? canonicalPdf?.pdfBuffer : srcIsPages ? pagesCanonicalPdf : rawBuf;
            if (!sourcePdfBuffer) {
            ...
            if (rendered.packaging === "zip") {
                ...
                outputFilename = `${(0, formats_1.stripExt)(srcFilename)}_${outputFormat.toLowerCase()}_pages.zip`;
            }
            else {
                contentType = (0, formats_1.contentTypeFor)(outputFormat);
                outputFilename = (0, formats_1.replaceExt)(srcFilename, (0, formats_1.extForFormat)(outputFormat));
            }
