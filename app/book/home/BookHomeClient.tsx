...
export function BookHomeClient() {
  ...
  const sessionRoute = useMemo(() => {
    ...
    const chapterId = `ch-${String(Math.max(1, currentProgress.chapter)).padStart(2, "0")}`;
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden text-slate-100">
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pb-24 pt-7 sm:px-6 sm:pt-8">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-lg text-slate-300">
              You're on a {dashboard.state.streakDays}-day streak. Keep it up.
            </p>
          </div>
          <span className="inline-flex items-center gap-2 rounded-full border border-amber-300/35 bg-amber-300/14 px-3.5 py-1.5 text-sm font-medium text-amber-100">
            ...
            {dashboard.state.streakDays} day streak
          </span>
        </div>
        ...
        <div className="mt-7 grid grid-cols-1 gap-4 lg:grid-cols-[1.55fr_1fr]">
          {currentBook && currentProgress ? (
            ...
            <div className="rounded-[30px] border border-white/10 bg-white/[0.03] p-6 text-slate-300">
              Add books in onboarding to start reading.
            </div>
          )}
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-3">
          <article className="rounded-3xl border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.03))] p-5 lg:col-span-2">
            <h3 className="text-lg font-semibold text-slate-100">Daily Insight</h3>
            <p className="mt-3 text-slate-200">{dailyInsight.takeaway}</p>
            <p className="mt-2 text-sm text-slate-400">
              Try this in real life: {dailyInsight.action}
            </p>
          </article>
          <GoalMeter
            goalMinutes={onboarding.dailyGoalMinutes}
            minutesReadToday={dashboard.state.minutesReadToday}
            onAddTenMinutes={() => dashboard.addReadMinutes(10)}
          />
        </div>
          ...
          <div className="mt-3 grid grid-cols-1 gap-2.5 md:grid-cols-3">
            {QUICK_REVIEW_PROMPTS.map((prompt) => (
              <button
                ...
                type="button"
                onClick={() => setShowQuickReviewModal(true)}
                className="rounded-2xl border border-white/12 bg-white/[0.03] px-3 py-3 text-left text-sm text-slate-200 transition hover:border-sky-300/30 hover:bg-sky-400/8"
              >
                {prompt}
              </button>
            ))}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {filteredRecentBooks.slice(0, 5).map(({ book, progress }) => (
              <BookMiniCard
                ...
                book={book}
                progress={progress}
                onOpen={() => dashboard.setCurrentBookId(book.id)}
              />
            ))}
          </div>
        </div>
        ...
        <div className="fixed bottom-4 left-4 right-4 z-40 md:hidden">
          <div className="flex items-center gap-2 rounded-2xl border border-sky-300/35 bg-[#0b1324]/95 p-2 shadow-[0_16px_32px_rgba(2,6,23,0.5)] backdrop-blur">
            <button
              ...
              className="flex-1 rounded-xl bg-gradient-to-r from-sky-500 to-cyan-400 px-3 py-2.5 text-sm font-semibold text-white"
            >
      ...
      <InfoModal
        ...
        title="Quick Review coming soon"
